...
try {
    ...
    const updateUser=await userModel.findByIdAndUpdate({_id:employee.userId},{name}
    )
    const editEmployee=await Employee.findByIdAndUpdate({_id:id},{
        maritalStatus,
        designation,
        department,
        salary,
    ...
    if(!updateUser || !editEmployee){
        return res.status(400).json({success:false, error:"Failed to update employee"})
    }
